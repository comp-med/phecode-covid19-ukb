...
chr="$(awk -v var="$SLURM_ARRAY_TASK_ID" -F '\t' 'NR == var {print $1}' input/Job.submission.REGENIE.step2.men.phecodes.txt)"
batch="$(awk -v var="$SLURM_ARRAY_TASK_ID" -F '\t' 'NR == var {print $2}' input/Job.submission.REGENIE.step2.men.phecodes.txt)"
...
if [[ $chr -eq 23 ]]; then
  ...
  --covarFile input/covariates.male.txt \
  --threads 120 \
  --bt \
  --spa 0.01 \
